import { Palak_backend } from "../../declarations/Palak_backend";
...
appointForm.addEventListener("submit", async (e) => {
  ...
  try {
    const getId = await Palak_backend.addTodo(obj.textarea);
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    const getAll = await Palak_backend.getAllTodo();
...
async function deleteTodo(userId) {
  try {
    ...
    await Palak_backend.deleteTodo(userId);
